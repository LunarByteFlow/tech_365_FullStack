...
const Create_InventoryScreen = async (req, res) => {
    ...
    if (!Inventory_Screens_ID || !Brand || !Model || !Screen_Size) {
        return res.status(400).json({ success: false, message: "Missing required fields: Inventory_Screens_ID, Brand, Model, Screen_Size." });
    }
    ...
    try {
        ...
        res.status(201).json({ success: true, message: "Inventory screen item created successfully." });
    } catch (error) {
...
const Get_AllInventoryScreens = async (req, res) => {
    try {
        ...
        if (result.recordset.length === 0) {
            return res.status(404).json({ success: false, message: "No inventory screen records found." });
        }
...
const Get_InventoryScreenById = async (req, res) => {
    ...
    if (!id) {
        return res.status(400).json({ success: false, message: "Inventory Screen ID is required in the URL." });
    }
    ...
    try {
        ...
        const result = await request.query("SELECT * FROM [Inventory_Screens] WHERE Inventory_Screens_ID = @Inventory_Screens_ID");

        if (result.recordset.length === 0) {
            return res.status(404).json({ success: false, message: "Inventory screen item not found." });
        }
...
const Update_InventoryScreen = async (req, res) => {
    ...
    if (!id) {
        return res.status(400).json({ success: false, message: "Inventory Screen ID is required in the URL for update." });
    }
    ...
    try {
        ...
        if (Ports_ !== undefined) { fieldsToUpdate.push("Ports_ = @Ports_"); request.input("Ports_", sql.VarChar, Ports_); }
        if (Stand !== undefined) { fieldsToUpdate.push("Stand = @Stand"); request.input("Stand", sql.VarChar, Stand); }
        ...
        if (result.rowsAffected[0] === 0) {
            return res.status(404).json({ success: false, message: "Inventory screen item not found or no changes made." });
        }

        res.status(200).json({ success: true, message: "Inventory screen item updated successfully." });
    } catch (error) {
...
const Delete_InventoryScreen = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ success: true, message: "Inventory screen item deleted successfully." });
    } catch (error) {
