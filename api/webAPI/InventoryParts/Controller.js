...
const Create_PartInventory = async (req, res) => {
    ...
    if (!Inventory_Parts_ID || !Brand || !Model || !Type_) {
        return res.status(400).json({ success: false, message: "Missing required fields: Inventory_Parts_ID, Brand, Model, Type_." });
    }
    ...
    try {
        ...
        res.status(201).json({ success: true, message: "Part inventory item created successfully." });
    } catch (error) {
...
const Get_AllPartInventory = async (req, res) => {
    try {
        ...
        if (result.recordset.length === 0) {
            return res.status(404).json({ success: false, message: "No part inventory records found." });
        }
...
const Get_PartInventoryById = async (req, res) => {
    ...
    if (!id) {
        return res.status(400).json({ success: false, message: "Part Inventory ID is required in the URL." });
    }
    ...
    try {
        ...
        const result = await request.query("SELECT * FROM [Inventory_Parts] WHERE Inventory_Parts_ID = @Inventory_Parts_ID");

        if (result.recordset.length === 0) {
            return res.status(404).json({ success: false, message: "Part inventory item not found." });
        }
...
const Update_PartInventory = async (req, res) => {
    ...
    if (!id) {
        return res.status(400).json({ success: false, message: "Part Inventory ID is required in the URL for update." });
    }
    ...
    try {
        ...
        if (Type_ !== undefined) { fieldsToUpdate.push("Type_ = @Type_"); request.input("Type_", sql.VarChar, Type_); }
        if (Comments !== undefined) { fieldsToUpdate.push("Comments = @Comments"); request.input("Comments", sql.VarChar, Comments); } // Comments field
        ...
        if (result.rowsAffected[0] === 0) {
            return res.status(404).json({ success: false, message: "Part inventory item not found or no changes made." });
        }

        res.status(200).json({ success: true, message: "Part inventory item updated successfully." });
    } catch (error) {
...
const Delete_PartInventory = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ success: true, message: "Part inventory item deleted successfully." });
    } catch (error) {
